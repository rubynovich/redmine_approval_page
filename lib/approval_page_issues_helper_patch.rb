    ...
    def show_detail_with_approval_page(detail, no_html=false, options={})
      if detail.property == 'watchers'
        if detail.prop_key == 'approver' && ((detail.old_value.to_s.split(',') - detail.value.to_s.split(',')).count > 0)
          ...
          return l(:message_remove_approver, name: user.name)[user.try(:gender).to_s].html_safe
          ...
          return (users.count > 1 ? l(:message_add_approvers, names: users.map(&:name).join(', ')) : l(:message_add_approver_gender, name: users.first.name)[users.first.try(:gender).to_s] ).html_safe
