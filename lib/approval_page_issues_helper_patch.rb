    ...
    def show_detail_with_approval_page(detail, no_html=false, options={})
      if detail.property == 'watchers'
        if detail.prop_key == 'approver' && ((detail.old_value.to_s.split(',') - detail.value.to_s.split(',')).count > 0)
          ...
          return l(:message_remove_approver_gender, name: user.name)["g#{user.try(:gender).to_s}".to_sym].gsub("%{name}",user.name).html_safe
          ...
          return (users.count > 1 ? l(:message_add_approvers, names: users.map(&:name).join(', ')) : l(:message_add_approver_gender, name: users.first.name)["g#{users.first.try(:gender).to_s}".to_sym].gsub("%{name}",users.first.name) ).html_safe
